...
# .env still lives at desktop_app/.env, so from clients/ we go one level up.
...
def play_song(query):
    """
    Search and start a track using the Spotify Web API.

    Important boundary rule:
    - this client talks to Spotify's API only
    - opening/waiting for the local desktop app belongs to integrations/spotify_desktop.py
    """
    try:
...
def play_playlist(playlist_name):
    """
    Start a playlist using the Spotify Web API.

    Important boundary rule:
    - this client talks to Spotify's API only
    - local desktop preparation belongs to integrations/spotify_desktop.py
    """
    try:
