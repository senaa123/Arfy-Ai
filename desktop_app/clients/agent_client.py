...
def ask_agent(text: str, session_id: str) -> dict:
    """
    Send user text to the agent service.

    Phase 1.5 change:
    - desktop no longer retrieves memories itself
    - desktop sends only the raw request context it truly owns
    - agent service now handles memory retrieval internally
    """
    try:
